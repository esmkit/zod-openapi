...
import { type ComponentsObject, createComponentResponseRef } from "./components";
import { createContent } from "./content";
import type { CreateDocumentOptions, ZodOpenApiResponseObject, ZodOpenApiResponsesObject } from "./document";
import { type SchemaState, createSchema } from "./schema";
import { isISpecificationExtension } from "./specificationExtension";

export const createResponseHeaders = (
  responseHeaders: oas31.HeadersObject | oas30.HeadersObject | AnyZodObject | undefined,
  ...
  if (isAnyZodType(responseHeaders)) {
    return Object.entries(responseHeaders.shape as ZodRawShape).reduce<NonNullable<oas31.ResponseObject["headers"]>>(
...
export const createBaseHeader = (schema: ZodType, components: ComponentsObject, documentOptions?: CreateDocumentOptions): oas31.BaseParameterObject => {
...
export const createComponentHeaderRef = (ref: string) => `#/components/headers/${ref}`;
...
export const createResponse = (
  ...
  const maybeHeaders = createResponseHeaders(headers, components, documentOptions);

  const response: oas31.ResponseObject = {
    ...rest,
    ...(maybeHeaders && { headers: maybeHeaders }),
    ...(content && {
      content: createContent(content, components, "output", [...subpath, "content"], documentOptions),
    }),
  };
...
export const createResponses = (
  ...
  Object.entries(responsesObject).reduce<oas31.ResponsesObject>(
    (acc, [statusCode, responseObject]: [string, ZodOpenApiResponseObject | oas31.ReferenceObject]): oas31.ResponsesObject => {
      if (isISpecificationExtension(statusCode)) {
        acc[statusCode] = responseObject;
        return acc;
      }
      acc[statusCode] = createResponse(responseObject, components, [...subpath, statusCode], documentOptions);
      return acc;
    },
    {},
  );
