...
import type { CreateDocumentOptions, ZodObjectInputType, ZodOpenApiParameters } from "./document";
import { type SchemaState, createSchema } from "./schema";

export const createComponentParamRef = (ref: string) => `#/components/parameters/${ref}`;
...
export const createBaseParameter = (
  ...
  const description = schema._def.zodOpenApi?.openapi?.description ?? schema._def.description;
...
export const createParamOrRef = (
  ...
  const paramType = zodSchema._def.zodOpenApi?.openapi?.param?.in ?? component?.in ?? type;
  const paramName = zodSchema._def.zodOpenApi?.openapi?.param?.name ?? component?.name ?? name;
  ...
  const baseParamOrRef = createBaseParameter(zodSchema, components, subpath, documentOptions);
...
const createParameters = (
  ...
  return Object.entries(zodObject).map(([key, zodSchema]: [string, ZodType]) =>
    createParamOrRef(zodSchema, components, [...subpath, key], type, key, documentOptions),
  );
};
...
const createRequestParams = (
  ...
  const pathParams = createParameters("path", requestParams.path, components, [...subpath, "path"], documentOptions);
  const queryParams = createParameters("query", requestParams.query, components, [...subpath, "query"], documentOptions);
  const cookieParams = createParameters("cookie", requestParams.cookie, components, [...subpath, "cookie"], documentOptions);
  const headerParams = createParameters("header", requestParams.header, components, [...subpath, "header"], documentOptions);

  return [...pathParams, ...queryParams, ...cookieParams, ...headerParams];
};

export const createManualParameters = (
  parameters: Array<oas31.ParameterObject | oas31.ReferenceObject | oas30.ParameterObject | oas30.ReferenceObject | ZodType> | undefined,
  components: ComponentsObject,
  subpath: string[],
  documentOptions?: CreateDocumentOptions,
): Array<oas31.ParameterObject | oas31.ReferenceObject> =>
  parameters?.map((param, index) => {
    if (isAnyZodType(param)) {
      return createParamOrRef(param, components, [...subpath, `param index ${index}`], undefined, undefined, documentOptions);
    }
...
export const createParametersObject = (
  ...
  const manualParameters = createManualParameters(parameters, components, subpath, documentOptions);
  const createdParams = createRequestParams(requestParams, components, subpath, documentOptions);
  const combinedParameters: oas31.OperationObject["parameters"] = [...manualParameters, ...createdParams];

  return combinedParameters.length ? combinedParameters : undefined;
};

export const getZodObject = (schema: ZodObjectInputType, type: "input" | "output"): AnyZodObject => {
