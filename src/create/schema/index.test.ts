...
const zodPreprocess = z.preprocess((arg) => (typeof arg === "string" ? arg.split(",") : arg), z.string());
...
type Lazy = Lazy[];
const zodLazy: z.ZodType<Lazy> = z.lazy(() => zodLazy.array()).openapi({ ref: "lazy" });
...
it("creates an output schema for zodType", () => {
  expect(createSchema(zodLazyComplex, createOutputState(), ["previous"])).toEqual(expectedZodLazyComplex);
});
...
describe("createSchema", () => {
  ...
  `("creates an output schema for $zodType", ({ schema, expected }) => {
    expect(createSchema(schema, createOutputState(), ["previous"])).toEqual(expected);
  });
  ...
  it("throws an error when an ZodEffect input component is referenced in an output", () => {
    ...
    expect(() => createSchema(outputSchema, outputState, ["previous"])).toThrowErrorMatchingInlineSnapshot(`
...
`);
  ...
  it("does not throw an error when a transform is generated with different types", () => {
    ...
    const result = createSchema(outputSchema, outputState, ["previous", "path"]);
  ...
  it("does not throw an error when a pipe is generated with different types", () => {
    ...
    const result2 = createSchema(outputSchema, outputState, ["previous", "path"]);
...
`);
  ...
  it("throws an error when a lazy schema which contains an effect is used in both an input and output", () => {
    ...
    expect(() => createSchema(ContainerSchema, inputState, ["previous"])).toThrowErrorMatchingInlineSnapshot(`
