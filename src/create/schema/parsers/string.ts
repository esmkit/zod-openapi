...
export const createStringSchema = (zodString: ZodString, state: SchemaState): Schema => {
  ...
  const contentEncoding = satisfiesVersion(state.components.openapi, "3.1.0") ? mapContentEncoding(zodStringChecks) : undefined;
...
type ZodStringCheckMap = {
  [kind in ZodStringCheck["kind"]]?: [Extract<ZodStringCheck, { kind: kind }>, ...Array<Extract<ZodStringCheck, { kind: kind }>>];
};

const getZodStringChecks = (zodString: ZodString): ZodStringCheckMap =>
  zodString._def.checks.reduce<ZodStringCheckMap>((acc, check: ZodStringCheck) => {
...
const mapPatterns = (zodStringChecks: ZodStringCheckMap): string[] => {
  ...
  const patterns: string[] = [...(regex ?? []), ...(startsWith ? [startsWith] : []), ...(endsWith ? [endsWith] : []), ...(includes ?? [])];

  return patterns;
};

const mapStartsWith = (zodStringChecks: ZodStringCheckMap): oas31.SchemaObject["pattern"] => {
...
const mapEndsWith = (zodStringChecks: ZodStringCheckMap): oas31.SchemaObject["pattern"] => {
...
const mapRegex = (zodStringChecks: ZodStringCheckMap): string[] | undefined => zodStringChecks.regex?.map((regexCheck) => regexCheck.regex.source);

const mapIncludes = (zodStringChecks: ZodStringCheckMap): string[] | undefined =>
...
const mapStringFormat = (zodStringChecks: ZodStringCheckMap): oas31.SchemaObject["format"] => {
...
const mapContentEncoding = (zodStringChecks: ZodStringCheckMap): string | undefined => {
