...
describe("createLazySchema", () => {
  it("throws an error when a lazy schema has no ref", () => {
    ...
    expect(() => createSchemaObject(lazy as ZodLazy<any>, createOutputState(), ["response"])).toThrow(
  ...
  it("throws errors when cycles without refs are detected", () => {
    ...
    expect(() => createSchemaObject(cycle1, createOutputState(), ["response"])).toThrow(
    ...
    expect(() => createSchemaObject(cycle2, createOutputState(), ["response"])).toThrow(
    ...
    expect(() => createSchemaObject(cycle3, createOutputState(), ["response"])).toThrow(
  ...
  it("creates an lazy schema when the schema contains a ref", () => {
    type Lazy = Lazy[];
    const lazy: z.ZodType<Lazy> = z.lazy(() => lazy.array()).openapi({ ref: "lazy" });
  ...
  it("supports registering the base schema", () => {
    ...
    const result = createObjectSchema(UserSchema as ZodObject<any, any, any, any, any>, undefined, state);
  ...
  it("creates a lazy schema which contains an effect", () => {
    ...
    const UserSchema = (PostSchema as ZodObject<any, any, any, any, any>).shape.user;
    ...
    expect(state.components.schemas.get(UserSchema)).toEqual(expectedUserComponent);
  });
});
