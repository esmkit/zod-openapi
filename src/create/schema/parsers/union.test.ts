...
describe("createUnionSchema", () => {
  ...
  it("creates an oneOf schema for a union if the document options unionOneOf is true", () => {
    ...
    const result = createUnionSchema(schema, createOutputState(undefined, { unionOneOf: true }));
  ...
  it("preferences individual unionOneOf over global setting", () => {
    ...
    const schema = z.union([z.string(), z.number()]).openapi({ unionOneOf: false });
  ...
  it("ignores optional values in a union", () => {
    const schema = z.union([z.string(), z.literal(undefined), z.undefined(), z.never()]);
