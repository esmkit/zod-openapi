...
import { type RefObject, type Schema, type SchemaState, createSchemaObject } from "../../schema";
...
>(
  ...
  const extendedSchema = createExtendedSchema(zodObject, previous?.zodType as ZodObject<T, UnknownKeys, Catchall, Output, Input>, state);
...
>(
  ...
  const diffShape = createShapeDiff(baseZodObject._def.shape() as ZodRawShape, zodObject._def.shape() as ZodRawShape);

  if (!diffShape) {
    return undefined;
  }

  const extendedSchema = createObjectSchemaFromShape(diffShape, diffOpts, state, true);
  ...
  if (schemaLength === 0) {
    return {
      type: "ref",
      schema: {
        $ref: createComponentSchemaRef(completeComponent.ref, state.documentOptions?.componentRefPath),
      },
      schemaObject: completeComponent.type === "complete" ? completeComponent.schemaObject : undefined,
      zodType: zodObject,
      effects,
    };
  }
...
const createDiffOpts = (baseOpts: AdditionalPropertyOptions, extendedOpts: AdditionalPropertyOptions): AdditionalPropertyOptions | undefined => {
...
const createShapeDiff = (baseObj: ZodRawShape, extendedObj: ZodRawShape): ZodRawShape | null => {
...
export const createObjectSchemaFromShape = (
  ...
  const additionalProperties = !isZodType(catchAll, "ZodNever") ? createSchemaObject(catchAll, state, ["additional properties"]) : undefined;
  ...
  return {
    ...
    effects: flattenEffects([...(properties?.effects ?? []), additionalProperties?.effects, required?.effects]),
  };
};
...
export const mapProperties = (shape: ZodRawShape, state: SchemaState): PropertyMap | undefined => {
