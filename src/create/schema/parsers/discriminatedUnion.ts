import type { AnyZodObject, ZodDiscriminatedUnion, ZodDiscriminatedUnionOption, ZodRawShape, ZodType, ZodTypeAny } from "zod";
...
export const createDiscriminatedUnionSchema = <Discriminator extends string, Options extends Array<ZodDiscriminatedUnionOption<Discriminator>>>(
  ...
  const discriminator = mapDiscriminator(schemaObjects, options, zodDiscriminatedUnion.discriminator, state);
...
const unwrapLiterals = (zodType: ZodType | ZodTypeAny | undefined, state: SchemaState): string[] | undefined => {
  ...
  for (const [index, zodObject] of zodObjects.entries()) {
    ...
    if (!componentSchemaRef) {
      if (state.documentOptions?.enforceDiscriminatedUnionComponents) {
        throw new Error(`Discriminated Union member ${index} at ${state.path.join(" > ")} is not registered as a component`);
      }
      return undefined;
    }
