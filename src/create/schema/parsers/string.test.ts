...
import { createOutputOpenapi3State, createOutputState } from "../../../testing/state";
...
describe("createStringSchema", () => {
  ...
  it("creates a string schema with multiple patterns and length checks", () => {
    ...
    const schema = z.string().min(10).includes("hello").startsWith("hello").regex(/^foo/).regex(/foo$/);
  ...
  it("does not support contentEncoding in 3.0.0", () => {
    ...
    const result = createStringSchema(z.string().base64(), createOutputOpenapi3State());
