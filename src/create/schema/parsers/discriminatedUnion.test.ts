...
describe("createDiscriminatedUnionSchema", () => {
  ...
  it("creates a oneOf schema with discriminator mapping when schemas with enums are registered manually", () => {
    ...
    const result = createDiscriminatedUnionSchema(schema, createOutputState(components));
  ...
  it("throws an error if enforceDiscriminatedUnionComponents is specified and a schema is not registered", () => {
    ...
    ).toThrowError(`Discriminated Union member 0 at some > path is not registered as a component`);
  });
});
