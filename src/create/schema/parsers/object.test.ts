...
describe("createObjectSchema", () => {
  ...
  it("supports catchall", () => {
    ...
    expect(createObjectSchema(schema, undefined, createOutputState())).toEqual(expected);
  });
...
describe("extend", () => {
  ...
  it("does not create an allOf schema when the base object has a catchall", () => {
    ...
    const object1 = z.object({ a: z.string() }).catchall(z.boolean()).openapi({ ref: "obj1" });
  ...
  it("creates an object with 2 required fields using a custom type", () => {
    ...
    const zodDate = z
      .union([
        ...
        z
          .string()
          .transform((str: string): Date => new Date(str)), // ignore validation
      ])
      .openapi({
        type: "string",
        format: "date",
