...
export const createNativeEnumSchema = <T extends EnumLike>(zodEnum: ZodNativeEnum<T>, state: SchemaState): Schema => {
...
export const getValidEnumValues = (enumValues: EnumLike) => {
  const keys = Object.keys(enumValues).filter((key) => typeof enumValues[enumValues[key] as number] !== "number");
  return keys.map((key) => enumValues[key] as number);
};

export const sortStringsAndNumbers = (values: Array<string | number>): StringsAndNumbers => ({
