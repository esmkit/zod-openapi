...
export const createIntersectionSchema = <T extends ZodTypeAny, U extends ZodTypeAny>(zodIntersection: ZodIntersection<T, U>, state: SchemaState): Schema => {
  const schemas = flattenIntersection(zodIntersection);
  const allOfs = schemas.map((schema, index) => createSchemaObject(schema, state, [`intersection ${index}`]));
  return {
    type: "schema",
    schema: {
