...
export const createRecordSchema = <Key extends KeySchema = ZodString, Value extends ZodTypeAny = ZodTypeAny>(
  zodRecord: ZodRecord<Key, Value>,
  state: SchemaState,
): Schema => {
  const additionalProperties = createSchemaObject(zodRecord.valueSchema as ZodTypeAny, state, ["record value"]);

  const keySchema = createSchemaObject(zodRecord.keySchema, state, ["record key"]);
  ...
  if ("enum" in renderedKeySchema && renderedKeySchema.enum) {
    return {
      ...
      schema: {
        type: "object",
        properties: (renderedKeySchema.enum as string[]).reduce<NonNullable<oas31.SchemaObject["properties"]>>((acc, key) => {
          acc[key] = additionalProperties.schema;
          return acc;
        }, {}),
        additionalProperties: false,
      },
      effects: flattenEffects([keySchema.effects, additionalProperties.effects, maybeEffects]),
    };
  }
  ...
  if (
    ...
    return {
      ...
      effects: flattenEffects([keySchema.effects, additionalProperties.effects]),
    };
  }
