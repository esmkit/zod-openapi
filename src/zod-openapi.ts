...
function parseTransformation({ zodRef, schemas, useOutput }: ParsingArgs<z.ZodTransformer<never> | z.ZodEffects<never>>): SchemaObject {
...
function parseObject({ zodRef, schemas, useOutput }: ParsingArgs<z.ZodObject<never, "passthrough" | "strict" | "strip">>): SchemaObject {
...
function parseBigInt({ zodRef, schemas }: ParsingArgs<z.ZodBigInt>): SchemaObject {
	return merge({ type: "integer" as SchemaObjectType, format: "int64" }, zodRef.description ? { description: zodRef.description } : {}, ...schemas);
}

function parseBoolean({ zodRef, schemas }: ParsingArgs<z.ZodBoolean>): SchemaObject {
	return merge({ type: "boolean" as SchemaObjectType }, zodRef.description ? { description: zodRef.description } : {}, ...schemas);
}

function parseDate({ zodRef, schemas }: ParsingArgs<z.ZodDate>): SchemaObject {
	return merge({ type: "string" as SchemaObjectType, format: "date-time" }, zodRef.description ? { description: zodRef.description } : {}, ...schemas);
}
...
function parseOptionalNullable({ schemas, zodRef, useOutput }: ParsingArgs<z.ZodOptional<OpenApiZodAny> | z.ZodNullable<OpenApiZodAny>>): SchemaObject {
...
function parseIntersection({ schemas, zodRef, useOutput }: ParsingArgs<z.ZodIntersection<z.ZodTypeAny, z.ZodTypeAny>>): SchemaObject {
	return merge(
		{
...
function parseUnion({ schemas, zodRef, useOutput }: ParsingArgs<z.ZodUnion<[z.ZodTypeAny, ...z.ZodTypeAny[]]>>): SchemaObject {
	return merge(
		{
			oneOf: (zodRef as z.ZodUnion<[z.ZodTypeAny, ...z.ZodTypeAny[]]>)._def.options.map((schema) => generateSchema(schema, useOutput)),
		},
		zodRef.description ? { description: zodRef.description } : {},
		...schemas,
	);
...
function parseDiscriminatedUnion({
	...
	return merge(
		{
			discriminator: {
				propertyName: (zodRef as z.ZodDiscriminatedUnion<string, z.ZodDiscriminatedUnionOption<string>[]>)._def.discriminator,
			},
			oneOf: Array.from((zodRef as z.ZodDiscriminatedUnion<string, z.ZodDiscriminatedUnionOption<string>[]>)._def.options.values()).map((schema) =>
...
function parseBranded({ schemas, zodRef }: ParsingArgs<z.ZodBranded<z.ZodAny, string>>): SchemaObject {
...
export function generateSchema(zodRef: OpenApiZodAny, useOutput?: boolean): SchemaObject {
	...
	const schemas = [zodRef.isNullable && zodRef.isNullable() ? { nullable: true } : {}, ...(Array.isArray(metaOpenApi) ? metaOpenApi : [metaOpenApi])];
